    ...
    if(cols) {
      ...
      if(this.data && this.data.length) {
        for(let i in this.data) {
          let row = this.data[i], td = []
          for(let j in cols) {
            let c = cols[j]
            if(!(c instanceof Object))
              td.push(h('td', {domProps: {innerHTML: row[j] instanceof Array ? row[j].join(', ') : e(row[j])}, key: j}))
            else if(!hide[j]) {
              ...
              if(!c.type) {
                ...
                if(c.href)
                  ...
                  p.domProps = {innerHTML: e(t)}
                  td.push(h('td', p))
                }
              } else
                td.push(h('table-cell', {props: {columns: this.tbl.columns, row, key: j, items: l, slaves: this.slaves}}))
            }
          }
          if(this.tbl.action) {
            let a = []
      ...
      tbl.push(h('tbody', this.data && this.data.map((r, i) => h('tr', {key: i}, r && r.map((c, j) => h('td', {domProps: c, key: j}))))))
